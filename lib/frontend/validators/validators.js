...
import paramsStore from 'lib/frontend/stores/params-store/params-store'
...
export default {
  ...
  email: (val, option, fn) => {
    val = val.trim()
    fn(!paramsStore.regexps.email.test(val) && val !== '' ? t('validators.invalid.email') : null)
  },

  emails: (val, option, fn) => {
    val = val.split(',').map((e) => e.trim().toLowerCase()).join(',')
    fn(!paramsStore.regexps.emails.test(val) && val !== '' ? t('validators.invalid.email') : null)
  },

  firstname: (val, option, fn) => {
    val = val.trim()
    fn(!paramsStore.regexps.names.test(val) && val !== '' ? t('validators.invalid.firstname') : null)
  },

  lastname: (val, option, fn) => {
    val = val.trim()
    fn(!paramsStore.regexps.names.test(val) && val !== '' ? t('validators.invalid.lastname') : null)
  },

  author: (val, option, fn) => {
    val = val ? val.trim() : ''
    fn(!paramsStore.regexps.names.test(val) && val !== '' ? t('validators.invalid.author') : null)
  },
  ...
  numeric: (val, option, fn) => {
    fn(!paramsStore.regexps.numeric.test(val) && val.trim() !== '' ? t('validators.invalid.number') : null)
  },

  password: (val, option, fn) => {
    fn(!paramsStore.regexps.password.test(val) && val.trim() !== '' ? t('invalid.password') : null)
  },
  ...
  time: (val, option, fn) => {
    fn(val.trim() !== '' && !paramsStore.regexps.time.test(val) ? t('validators.invalid.time') : null)
  },

  forum: (val, option, fn) => {
    fn(val.trim() !== '' && !paramsStore.regexps.forum.name.test(val) ? t('validators.invalid.forum') : null)
  }
