...
var middlewares = require('lib/backend/middlewares/forum-middlewares')
var roles = require('lib/backend/models/forum/roles')
var reservedNames = require('lib/backend/models/forum/reserved-names')
var visibilities = require('lib/backend/models/forum/visibilities')
...
app.get('/forum/roles', restrict, function (req, res) {
  if (!req.user) return res.status(404).send()
  log('Handling forum roles')
  return res.status(200).json(roles)
})

app.get('/forum/reserved-names', restrict, function (req, res) {
  if (!req.user) return res.status(404).send()
  log('Handling forum reserved-names')
  return res.status(200).json(reservedNames)
})

app.get('/forum/visibilities', restrict, function (req, res) {
  if (!req.user) return res.status(404).send()
  log('Handling forum visibilities')
  return res.status(200).json(visibilities)
})
