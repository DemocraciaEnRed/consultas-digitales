...
var Flag = new Schema({
  author: { type: ObjectId, ref: 'User', required: true },
  value: { type: String, enum: [ 'spam', 'useful', 'positive', 'negative', 'to do' ], required: true },
  createdAt: { type: Date, default: Date.now }
})
...
var CommentSchema = new Schema({
  ...
  flags: [ Flag ],
  // The Administration flags for this comment
  adminMarks: [{ type: String }],
  createdAt: { type: Date, default: Date.now },
  editedAt: { type: Date }
})
...
CommentSchema.methods.mark = function (value, cb) {
  const adminMarks = this.adminMarks
  if(this.adminMarks){
    if(!this.adminMarks.find(m => m === value)) this.adminMarks.push(value)
  } else {
    this.adminMarks = [value]
  }
  this.save(cb)
}


CommentSchema.methods.unmark = function (value, cb) {
  let aux = this.adminMarks.filter( flag => {
    return flag != value
  })
  this.adminMarks = aux
  if (cb) this.save(cb)
