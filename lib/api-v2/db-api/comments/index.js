...
const privileges = require('lib/backend/privileges/forum')
 ...
 * @return {promise}
 */
exports.adminlist = function adminlist (opts) {
  opts = opts || {}

  return find()
    .where({ reference: opts.topicId })
    .populate(scopes.ordinary.populate)
    .select('id text createdAt editedAt reference flags score repliesCount replies author votes context adminMarks')
    .limit(opts.limit)
    .skip((opts.page - 1) * opts.limit)
    .sort(opts.sort)
    .exec()
    .then((comments) => comments.map((comment) => {
      return scopes.ordinary.exposeToAdmin(comment, opts.user)
    }))
}
 ...
 * @return {promise}
 */
exports.mark = function mark (opts) {
  const id = opts.id
  ...
  return find()
    ...
    .where({ _id: id })
    .populate(scopes.ordinary.populate)
    .select(scopes.ordinary.selectToAdmin)
    .exec()
    .then(doMark.bind(null, mark))
    .then((comment) => scopes.ordinary.expose(comment, user))
}


function doMark (value, comment) {
  return new Promise((resolve, reject) => {
    comment.mark(value, function (err) {
      if (err) return reject(err)
      resolve(comment)
    })
  })
 ...
 * @return {promise}
 */
exports.unmark = function unmark (opts) {
  const id = opts.id
  const mark = opts.mark
  const user = opts.user
  ...
  return find()
    ...
    .select(scopes.ordinary.selectToAdmin)
    .exec()
    .then(doUnmark.bind(null, mark))
    .then((comment) => scopes.ordinary.expose(comment, user))
}


function doUnmark (value, comment) {
  return new Promise((resolve, reject) => {
    comment.unmark(value, function (err) {
      if (err) return reject(err)
      resolve(comment)
    })
  })
